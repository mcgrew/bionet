...
package edu.purdue.cc.sysnet;

import edu.purdue.cc.sysnet.ui.*;
...
public class SysNet {
	...
	public static void main ( String [ ] args ) {
		...
		Getopt g = new Getopt( "SysNet", args, "dg:hv" );
		String arg;
		int c;
		...
		Settings settings = new Settings( defaultSettings,
			System.getProperty( "user.home" ) + "/.sysnet/settings.xml" );
		...
		while (( c = g.getopt( )) != -1 ) {
			switch( c ) {
				...
				case 'h':
					...
					System.out.println( "java SysNet [-d|h] [-g geometry]" );
		...
		try {
			...
			Logger logger = Logger.getLogger( SysNet.class );
			logger.debug( 
